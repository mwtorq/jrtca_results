...
if __name__ == "__main__":
    ...
    for year in sorted(by_year.keys(), reverse=True):
        ...
        for trial_name in sorted_trials:
            ...
            # Note: class_name may already include section (from find_nearest_section_and_class)
            # Use class_number as part of the key to ensure classes with same name but different numbers are separate
            by_class = defaultdict(list)

            

            # First pass: collect all classes to find height categories in the section
            # This helps us split generic "ADULT RACING CHAMPION" classes by height
            section_height_categories = defaultdict(set)
            for result in trial_results_list:

                class_name_lower = (result.class_name or "").lower()
                section = result.section or ""
                
                # Extract height from class name
                if '10"' in result.class_name or '10″' in result.class_name or '10 up to 12½' in class_name_lower or '10 up to 12.5' in class_name_lower:
                    section_height_categories[section].add("10_to_12.5")
                elif 'over' in class_name_lower and ('12½"' in result.class_name or '12.5' in class_name_lower or '12½' in result.class_name) and ('15"' in result.class_name or '15″' in result.class_name):
                    section_height_categories[section].add("over_12.5_to_15")
                elif '12½"' in result.class_name or '12.5' in class_name_lower or '12½' in result.class_name:
                    if '15"' in result.class_name or '15″' in result.class_name:
                        section_height_categories[section].add("over_12.5_to_15")
                    else:

                        section_height_categories[section].add("10_to_12.5")
                elif '15"' in result.class_name or '15″' in result.class_name:
                    section_height_categories[section].add("over_12.5_to_15")
            
            # Second pass: assign results to classes with height specifications
            # Track results by class name to detect when we need to split
            results_by_class_name = defaultdict(list)
            for result in trial_results_list:
                results_by_class_name[(result.section or "", result.class_name or "")].append(result)
            
            for result in trial_results_list:
                # Extract height/age specifications from class name to help split classes
                # that should be separate (e.g., different height categories)
                height_spec = None
                class_name_lower = (result.class_name or "").lower()
                section = result.section or ""
                
                # Check for height specifications in class name
                if '10"' in result.class_name or '10″' in result.class_name or '10 up to 12½' in class_name_lower or '10 up to 12.5' in class_name_lower:
                    height_spec = "10_to_12.5"
                elif '12½"' in result.class_name or '12.5' in class_name_lower or '12½' in result.class_name:
                    if 'over' in class_name_lower or '15' in result.class_name:
                        height_spec = "over_12.5_to_15"
                    else:
                ...
                # Special handling: "ADULT RACING CHAMPION" and "VETERAN RACING CHAMPION" should always be split by height
                # If class name is generic, check if there are multiple Champions/Reserves that need splitting
                if not height_spec and ('adult racing champion' in class_name_lower or 'veteran racing champion' in class_name_lower):
                    # Check how many results have this class name
                    same_class_results = results_by_class_name.get((section, result.class_name or ""), [])
                    champion_results = [r for r in same_class_results if r.placement and 'champion' in (r.placement or "").lower()]
                    reserve_results = [r for r in same_class_results if r.placement and 'reserve' in (r.placement or "").lower()]
                    
                    # If there are multiple Champions or Reserves, they need to be split by height
                    if len(champion_results) > 1 or (len(champion_results) > 0 and len(reserve_results) > 1):
                        # Check if this section has known height categories
                        section_heights = list(section_height_categories.get(section, set()))
                        if len(section_heights) >= 2:
                            # Multiple height categories exist - assign based on order
                            # Find this result's index among same-class results
                            result_index = same_class_results.index(result) if result in same_class_results else 0
                            # Alternate between height categories
                            height_spec = section_heights[result_index % len(section_heights)]
                        elif len(section_heights) == 1:
                            # Only one height category - use it
                            height_spec = section_heights[0]
                        else:
                            # No height categories found - use default split
                            # Assume standard two height categories
                            result_index = same_class_results.index(result) if result in same_class_results else 0
                            height_spec = "10_to_12.5" if result_index % 2 == 0 else "over_12.5_to_15"
                
                # Create a unique class key using division, section, class_number, class_name, and height_spec
                # Use a tuple for the key to ensure proper grouping
                class_key_tuple = (
                    ...
                    result.class_name or "",
                    height_spec or ""  # Add height specification to key to split classes
                )
            ...
            # Sort classes by class number (numeric), then by division/section/class_name/height_spec
            def get_class_sort_key(class_key_tuple: tuple, results: List[TrialResult]) -> tuple:

                division, section, class_num, class_name, height_spec = class_key_tuple
                # Sort by class number (numeric) first, then division, section, class_name, height_spec
                # Classes without numbers go after numbered classes (999999), sorted alphabetically
                # Use int() to ensure numeric sorting (not string sorting)
                class_num_sort = int(class_num) if class_num is not None else 999999

                # For classes without numbers, use class_name for secondary sort
                # For numbered classes, class_num_sort will be the primary sort
                return (class_num_sort, division or "", section or "", class_name or "", height_spec or "")
            
            # Sort classes
            sorted_classes = sorted(by_class.items(), key=lambda x: get_class_sort_key(x[0], x[1]))
            

            # Display results grouped by class

            for class_key, class_results in sorted_classes:
                # Remove duplicates within the class (same dog, same placement, same owner)
                ...
                # But first, check if multiple results with same placement are actually different dogs
                # (which would indicate a tie that wasn't marked)
                validated_results = []
                ...
                for base_placement, results_list in placement_counts.items():
                    ...
                    if non_ties:
                        ...
                        if len(non_ties) == 1:
                            ...
                            # If all results are for the same dog, it's a duplicate - keep only first
                            if len(unique_dogs) == 1:
                                validated_results.append(non_ties[0])

                            else:

                                # Different dogs with same placement - these are unmarked ties
                                # Mark them as ties by updating their placement
                                for result in non_ties:
                                    if result.placement and "(tie)" not in result.placement.lower():
                                        result.placement = f"{result.placement} (tie)"
                                    validated_results.append(result)
                
                # Validation: If there's a Champion, there shouldn't be a 1st place (Champion is the top placement)
                # If there's a Best, there shouldn't be a 1st place either
                has_champion = any(r.placement and 'champion' in r.placement.lower() for r in validated_results)

                has_best = any(r.placement and 'best' in r.placement.lower() and 'champion' not in r.placement.lower() for r in validated_results)

                
                if has_champion or has_best:
                    # Remove 1st, 2nd, etc. placements - Champion/Best is the top placement
                    validated_results = [r for r in validated_results if not (r.placement and re.match(r'^1(st)?', r.placement, re.IGNORECASE))]
                ...
                # Check for multiple Champions/Reserves in the same class - this might indicate separate classes
                # that got grouped together (e.g., different height categories)
                champion_count = sum(1 for r in class_results if r.placement and 'champion' in r.placement.lower())
                reserve_count = sum(1 for r in class_results if r.placement and 'reserve' in r.placement.lower())
                best_count = sum(1 for r in class_results if r.placement and 'best' in r.placement.lower() and 'champion' not in r.placement.lower())
                
                # Special handling: "ADULT RACING CHAMPION" and "VETERAN RACING CHAMPION" should always be split by height
                class_name_lower = (class_results[0].class_name or "").lower() if class_results else ""
                is_racing_champion = ('adult racing champion' in class_name_lower or 'veteran racing champion' in class_name_lower)
                has_multiple_champions = champion_count > 1 or (champion_count > 0 and reserve_count > 1)
                
                # If this is a racing champion class with multiple champions/reserves, split by height
                if is_racing_champion and has_multiple_champions:
                    # Look at nearby classes in the same section to determine height categories
                    section = class_results[0].section if class_results else ""
                    # Get all classes in this section to find height patterns
                    section_classes = [c for c in sorted_classes if c[0][1] == section]  # c[0] is class_key_tuple, [1] is section
                    
                    # Find height categories from nearby classes
                    height_categories = []
                    for other_class_key, other_class_results in section_classes:
                        other_class_name = other_class_key[3] if len(other_class_key) > 3 else ""
                        other_class_name_lower = other_class_name.lower()
                        if '10"' in other_class_name or '10″' in other_class_name or '10 up to 12½' in other_class_name_lower:
                            if "10_to_12.5" not in height_categories:
                                height_categories.append("10_to_12.5")
                        elif 'over' in other_class_name_lower and ('12½"' in other_class_name or '12.5' in other_class_name_lower) and ('15"' in other_class_name or '15″' in other_class_name):
                            if "over_12.5_to_15" not in height_categories:
                                height_categories.append("over_12.5_to_15")
                    
                    # If we found height categories, split the results
                    if len(height_categories) >= 2 and len(class_results) >= 2:
                        # Split results into separate classes by height
                        # For now, we'll assign them to different height categories
                        # This is a simplified approach - ideally we'd look at context to determine which result goes to which height
                        split_results = defaultdict(list)
                        for i, result in enumerate(class_results):
                            # Alternate or split based on some heuristic
                            # Simple approach: split evenly
                            height_idx = i % len(height_categories)
                            height_spec = height_categories[height_idx]
                            split_results[height_spec].append(result)
                        
                        # Replace the single class with split classes
                        # We'll need to update the class_key and create new entries
                        # For now, mark this class for special handling
                        pass  # This requires more complex restructuring
                ...
                if class_results[0].division:
                    class_parts.append(class_results[0].division)
                
                # Add section if available (section and class_name are now stored separately)
                if class_results[0].section:
                ...
                # Always show class number when available for consistency
                class_name_display = class_results[0].class_name or ""
                ...
                if class_name_display:
                    ...
                    if class_results[0].class_number is not None:
                        ...
                        # Always display class number when available
                        class_parts.append(f"Class {class_results[0].class_number}: {class_name_display}")
...
def main():
    ...
    for link_text, url, year in trial_links:
        ...
        if is_archive:
            ...
            results = parse_individual_trial_page(url, link_text, None, year)
    ...
    report = generate_trial_results_report(all_trial_results, catalog_dogs)
