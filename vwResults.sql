CREATE OR ALTER VIEW [sResults].[vwResults] AS
	...
	OUTER APPLY (SELECT STRING_AGG(CAST(ss.RelatedDogName AS NVARCHAR(MAX)),', ') AS Sibling FROM ((SELECT DISTINCT ss.RelatedDogName FROM [TrialResults].[sResults].[Relationship] ss (NOLOCK) WHERE d.DogID=ss.DogID AND ISNULL(ss.RelationshipType,'')='Sibling') UNION SELECT DISTINCT ss.RelatedDogName + ' (half)' FROM [TrialResults].[sResults].[Relationship] ss (NOLOCK) WHERE d.DogID=ss.DogID AND ISNULL(ss.RelationshipType,'') LIKE '%Half%Sibling%') ss) ss
	OUTER APPLY (SELECT STRING_AGG(CAST(sgs.RelatedDogName AS NVARCHAR(MAX)),', ') AS Grandsire FROM (SELECT DISTINCT sgs.RelatedDogName FROM [TrialResults].[sResults].[Relationship] sgs (NOLOCK) WHERE d.DogID=sgs.DogID AND ISNULL(sgs.RelationshipType,'') IN ('','Grandsire')) sgs) sgs
	OUTER APPLY (SELECT STRING_AGG(CAST(sgd.RelatedDogName AS NVARCHAR(MAX)),', ') AS Granddam FROM (SELECT DISTINCT sgd.RelatedDogName FROM [TrialResults].[sResults].[Relationship] sgd (NOLOCK) WHERE d.DogID=sgd.DogID AND ISNULL(sgd.RelationshipType,'') IN ('','Granddam')) sgd) sgd
	OUTER APPLY (SELECT STRING_AGG(CAST(sggs.RelatedDogName AS NVARCHAR(MAX)),', ') AS GreatGrandsire FROM (SELECT DISTINCT sggs.RelatedDogName FROM [TrialResults].[sResults].[Relationship] sggs (NOLOCK) WHERE d.DogID=sggs.DogID AND ISNULL(sggs.RelationshipType,'') IN ('','Great-Grandsire')) sggs) sggs
	OUTER APPLY (SELECT STRING_AGG(CAST(sggd.RelatedDogName AS NVARCHAR(MAX)),', ') AS GreatGranddam FROM (SELECT DISTINCT sggd.RelatedDogName FROM [TrialResults].[sResults].[Relationship] sggd (NOLOCK) WHERE d.DogID=sggd.DogID AND ISNULL(sggd.RelationshipType,'') IN ('','Great-Granddam')) sggd) sggd
