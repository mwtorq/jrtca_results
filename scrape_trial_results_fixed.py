#!/usr/bin/env python3
"""
Scrape trial results from JRTCA Yearbook, JRTCC, local files, and Trial Vault.
"""
...
def configure_console_output() -> None:
    """Use UTF-8 console output so Trial Vault filenames print safely on Windows."""
...
def normalize_division_name(division: str) -> str:
    ...
    division_map = {
        ...
        'SUPER EARTH GO-TO-GROUND': 'SUPER EARTH',
...
class TrialResult:
    ...
    time: Optional[str] = None  # Event time from Trial Vault (racing, GTG, etc.)
    sire: Optional[str] = None
    dam: Optional[str] = None
    nonsanctioned_event: bool = False
...
TRIALVAULT_BASE_URL = "https://v3.trialvault.dog"
TRIALVAULT_DAY_NAMES = {"Friday", "Saturday", "Sunday"}
TRIALVAULT_DAY_ABBREV = {
    "fri.": "Friday",
    "fri": "Friday",
    "sat.": "Saturday",
    "sat": "Saturday",
...
TRIALVAULT_TOP_DIVISIONS = {
    ...
    "Rally Obedience",
    "Brush Hunt",
    "Jack in the Shack (Non-Sanctioned)",
    "Lure Coursing (Non-Sanctioned)",
    "Thunder Tunnel (Non-Sanctioned)",
    "Top Gun",
    "Doggie Fun Zone (Non-Sanctioned)",
}


def prompt_trialvault_credentials() -> Tuple[str, str]:
    """Prompt for Trial Vault login credentials."""
    print("Trial Vault login required (https://v3.trialvault.dog/)")
...
def prompt_trialvault_results_url() -> str:
    ...
    print()
    print("Enter the Trial Vault results URL.")
    print("Example: https://v3.trialvault.dog/events/73/carolinas-spring-classic-2026/scores")
    url = input("Results URL: ").strip()
    while not url:
        url = input("Results URL (required): ").strip()
    return normalize_trialvault_results_url(url)


def normalize_trialvault_results_url(url: str) -> str:
    """Normalize a Trial Vault event/results URL."""
    url = url.strip()
    if not url.startswith("http"):
        url = urljoin(TRIALVAULT_BASE_URL, url)
    parsed = urlparse(url)
    if "trialvault.dog" not in parsed.netloc:
        raise ValueError(f"URL must be on trialvault.dog, got: {url}")
    if not url.rstrip("/").endswith("/scores"):
        url = url.rstrip("/") + "/scores"
    return url


def trialvault_login(session: requests.Session, email: str, password: str) -> bool:
    """Log in to Trial Vault and retain session cookies."""
    login_url = urljoin(TRIALVAULT_BASE_URL, "/accounts/login")
    try:
        response = session.get(login_url, timeout=30)
        response.raise_for_status()
    except Exception as e:
        print(f"ERROR: Could not load Trial Vault login page: {e}")
        return False

    soup = BeautifulSoup(response.text, "html.parser")
    csrf_input = soup.find("input", {"name": "csrfmiddlewaretoken"})
    if not csrf_input or not csrf_input.get("value"):
        print("ERROR: Could not find CSRF token on Trial Vault login page")
        return False
    ...
    try:
        ...
        print(f"ERROR: Trial Vault login request failed: {e}")
        return False

    if "/accounts/login" in response.url:
        print("ERROR: Trial Vault login failed. Check email and password.")
        return False

    print("Logged in to Trial Vault successfully.")
    return True


def fetch_trialvault_page(session: requests.Session, url: str, retries: int = 3) -> Optional[str]:
    """Fetch a Trial Vault page using an authenticated session."""
    for attempt in range(retries):
        try:
...
def fetch_trialvault_event_metadata(
    session: requests.Session, results_url: str
) -> Tuple[str, str, str]:
    """Fetch trial name, date string, and year from the Trial Vault event page."""
    event_url = re.sub(r"/scores/?$", "", results_url.rstrip("/")) + "/"
    trial_name = "Unknown Trial"
    date_str = ""
    year = str(time.localtime().tm_year)

    html_content = fetch_trialvault_page(session, event_url)
    if not html_content:
        return trial_name, date_str, year

    soup = BeautifulSoup(html_content, "html.parser")
    title_el = soup.find("h1", class_=re.compile("event-title")) or soup.find("h1")
    if title_el:
        trial_name = title_el.get_text(strip=True)

    year_match = re.search(r"(20\d{2})", trial_name)
    if year_match:
        year = year_match.group(1)

    page_text = soup.get_text(" ", strip=True)
    date_patterns = [
        r"(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\.\s+[A-Za-z]+\s+\d{1,2}\s*-\s*(?:Mon|Tue|Wed|Thu|Fri|Sat|Sun)\.\s+[A-Za-z]+\s+\d{1,2},?\s+\d{4}",
...
def clean_trialvault_dog_name(raw_name: str) -> str:
    """Strip Trial Vault entry prefix from a dog name."""
...
def get_trialvault_table_headers(table) -> List[str]:
    """Build a per-column header list, expanding colspan cells."""
    ...
    for th in header_row.find_all("th"):
        ...
        if colspan > 1 and text == "Rank":
            headers.extend(["RankIcon", "Rank"])
        elif colspan > 1:
...
def parse_trialvault_table_row(
    headers: List[str],
    cells: List[str],
) -> Optional[Tuple[str, str, str, Optional[str]]]:
    """Parse a Trial Vault results table row into placement, dog, owner, and optional time."""
    if not headers or not cells:
        return None

    width = min(len(headers), len(cells))
    row = {headers[i]: cells[i].strip() for i in range(width)}
    if row.get("Class"):
        return None

    placement = row.get("Rank", "").strip()
    if not placement:
        placement = row.get("Position", "").strip()
    dog_name = clean_trialvault_dog_name(row.get("Name", ""))
    owner = row.get("Owners", "").strip() or row.get("Parents", "").strip()
    event_time = row.get("Time", "").strip() or None

    if not dog_name or not placement:
        return None

    return placement, dog_name, owner, event_time


def _title_is_nonsanctioned(title: str) -> bool:
    return bool(title and "non-sanctioned" in title.lower())


def _is_trialvault_day_title(title: str) -> bool:
    """True when a Trial Vault section title marks a trial day, not a division."""
    if not title:
        return False
    day_key = title.strip().lower()
    if day_key in TRIALVAULT_DAY_ABBREV or title in TRIALVAULT_DAY_NAMES:
        return True
    if re.search(r"\bday\s+\d+\s*:", title, re.I):
        return True
    return False


def _trialvault_section_depth(el) -> int:
    """Return nesting depth from the nearest ancestor cl-section-N wrapper."""
    parent = getattr(el, "parent", None)
    while parent is not None:
        for cls in parent.get("class", []):
            match = re.fullmatch(r"cl-section-(\d+)", cls)
            if match:
...
def _trialvault_effective_division(
    current_division: str | None,
    current_section: str | None,
) -> str | None:
    """Choose the division label that should apply to classes in the current context."""
    if current_section and current_division:
        if "non-sanctioned" in current_division.lower():
            return current_section
    return current_division or current_section


def _trialvault_normalize_division_name(raw: str | None) -> str | None:
    """Normalize a Trial Vault section title to a canonical division name."""
    if not raw:
        return None
    upper = raw.upper().strip()
    if "SUPER EARTH" in upper:
        return normalize_division_name("SUPER EARTH")
    if "BARN HUNT" in upper or "BARN-HUNT" in upper:
        return normalize_division_name("BARN HUNT")
    return normalize_division_name(upper)


def _trialvault_class_division_override(class_name: str | None, division_name: str | None) -> str | None:
    """Use class wording to correct mis-assigned Trial Vault divisions."""
    if not class_name:
        return division_name
    class_lower = class_name.lower()
    if re.search(r"\bsuper\s+earth\b", class_lower):
        return normalize_division_name("SUPER EARTH")
    if re.search(r"\bbarn[\s-]*hunt\b", class_lower):
        return normalize_division_name("BARN HUNT")
    return division_name


def parse_trialvault_scores_html(
    soup: BeautifulSoup,
    trial_name: str,
    date_str: str,
    year: str,
) -> Tuple[List[TrialResult], Dict[str, str]]:
    """Parse Trial Vault scores HTML into trial result entries."""
    main = soup.find(id="main")
    if not main:
        print("ERROR: Could not find main content in Trial Vault results page")
        return [], {}

    current_day = None
    current_division = None
    current_section = None
    current_trial_subname = None
    current_nonsanctioned = False
    results: List[TrialResult] = []
    base_trial_name = trial_name
    ...
    for el in main.descendants:
        ...
        if el.name == "div" and "cl-section-description" in classes:
            desc = el.get_text(strip=True)
            if desc and re.search(r"\bcelebration\b", desc, re.I):
                current_trial_subname = desc
            continue

        if el.name != "div":
            continue

        if "cl-section-title" in classes:
            title = el.get_text(strip=True)
            if _is_trialvault_day_title(title):
                day_key = title.strip().lower()
                if day_key in TRIALVAULT_DAY_ABBREV:
                    current_day = TRIALVAULT_DAY_ABBREV[day_key]
                elif title in TRIALVAULT_DAY_NAMES:
                    current_day = title
                else:
                    for abbrev, day_name in TRIALVAULT_DAY_ABBREV.items():
                        if abbrev.rstrip(".") in day_key:
                ...
                current_trial_subname = None
                current_nonsanctioned = False
            elif title in TRIALVAULT_TOP_DIVISIONS:
                current_division = title
                current_section = None
                current_nonsanctioned = _title_is_nonsanctioned(title)
            else:
                depth = _trialvault_section_depth(el)
                if depth <= 1:
                    current_division = title
                    current_section = None
                    current_nonsanctioned = _title_is_nonsanctioned(title)
                    ...
                    if _title_is_nonsanctioned(title):
                        current_nonsanctioned = True
            continue

        if "cl-class-header" not in classes:
            continue

        title_el = el.find(class_="cl-class-title")
        number_el = el.find(class_="cl-class-number")
        class_name = title_el.get_text(strip=True) if title_el else None
        class_number_text = number_el.get_text(strip=True) if number_el else None
        class_number = int(class_number_text) if class_number_text and class_number_text.isdigit() else None

        if el.find("i", title=re.compile("no entries", re.I)):
            continue

        collapse_link = title_el.find("a", href=True) if title_el else None
        collapse_id = collapse_link["href"].lstrip("#") if collapse_link else None
        collapse = main.find(id=collapse_id) if collapse_id else None
        table = collapse.find("table") if collapse else None
        if not table:
            continue

        headers = get_trialvault_table_headers(table)
        if not headers or headers[0] == "DQ:" or "Class" in headers:
            continue
        ...
        effective_division = _trialvault_effective_division(current_division, current_section)
        division_name = _trialvault_normalize_division_name(effective_division)
        division_name = _trialvault_class_division_override(class_name, division_name)
        section_name = current_section

        display_trial_name = current_trial_subname or base_trial_name
        if current_day:
            day_suffix = f" - {current_day}"
            if day_suffix not in display_trial_name:
                display_trial_name = f"{display_trial_name}{day_suffix}"

        result_date = date_str
        if current_day:
            if result_date:
                if current_day not in result_date:
                    result_date = f"{current_day} ({result_date})"
            else:
                result_date = current_day

        for cells in data_rows:
            parsed = parse_trialvault_table_row(headers, cells)
            if not parsed:
                continue
            placement, dog_name, owner, event_time = parsed
            results.append(
                TrialResult(
                    trial_name=display_trial_name,
                    date=result_date,
                    year=year,
                    ...
                    owner=owner or None,
                    time=event_time,
                    nonsanctioned_event=current_nonsanctioned,
                )
            )
...
def sanitize_trial_filename(trial_name: str) -> str:
    """Sanitize a trial name for use in debug file names."""
    safe_name = re.sub(r'[<>:"/\\|?*\x00-\x1f]', '_', trial_name or "")
    safe_name = safe_name.replace(' ', '_').replace('&', 'and').replace("'", '')
    safe_name = safe_name[:50]
    return safe_name or "unknown_trial"


def normalize_trial_year(year: str, trial_name: str = "", results_url: str = "") -> str:
    """Return a four-digit year for trial result file subdirectories."""
    if year and re.fullmatch(r"\d{4}", str(year).strip()):
        return str(year).strip()
    for source in (trial_name, results_url):
...
def get_trial_debug_file_paths(
    trial_name: str,
    year: str,
    prefix: str = "debug_trial",
) -> Tuple[str, str, str]:
    """Return (year_dir, html_path, raw_text_path) for a trial in a year subdirectory."""
    year_dir = normalize_trial_year(year, trial_name=trial_name)
    os.makedirs(year_dir, exist_ok=True)
    safe_trial_name = sanitize_trial_filename(trial_name)
    html_path = os.path.join(year_dir, f"{prefix}_{safe_trial_name}.html")
    raw_text_path = os.path.join(year_dir, f"{prefix}_{safe_trial_name}_raw.txt")
    return year_dir, html_path, raw_text_path


def build_trialvault_raw_text(html_content: str, results_url: str, trial_name: str) -> str:
    """Strip Trial Vault HTML to normalized text for local storage."""
    soup = BeautifulSoup(html_content, "html.parser")
...
def save_trialvault_debug_files(
    html_content: str,
    trial_name: str,
    year: str,
    results_url: str,
) -> str:
    """Save Trial Vault HTML and stripped text in the event year subdirectory."""
    year_dir, debug_file, raw_text_file = get_trial_debug_file_paths(
        trial_name, year, prefix="debug_trialvault"
    )
    ...
    raw_text = build_trialvault_raw_text(html_content, results_url, trial_name)
    with open(raw_text_file, "w", encoding="utf-8") as f:
        f.write(raw_text)
    print(f"Saved stripped text to {raw_text_file}")
    print(f"Year folder: {year_dir}")
    print(f"Source URL: {results_url}")
    return debug_file


def scrape_trialvault_single() -> List[TrialResult]:
    """Prompt for credentials and trial URL, then scrape and load one Trial Vault event."""
    print("=" * 80)
    print("TRIAL VAULT - SINGLE EVENT RESULTS SCRAPER")
    print("=" * 80)
    print()

    email, password = prompt_trialvault_credentials()
    results_url = prompt_trialvault_results_url()
    print()
    ...
    print(f"Event: {trial_name}")
    if date_str:
        print(f"Date: {date_str}")
    print(f"Year: {year}")

    debug_html_path = save_trialvault_debug_files(html_content, trial_name, year, results_url)

    print("Parsing results from page HTML...")
    results, judges_info = parse_trialvault_scores_html(soup, trial_name, date_str, year)
    print(f"Extracted {len(results)} placements")

    if not results:
        print("WARNING: No results were extracted. Check the URL and page access.")
        sys.exit(1)

    print()
    print("Generating report...")
    report = generate_trial_results_report(results, {}, {trial_name: judges_info} if judges_info else None)
    ...
    try:
        from populate_trialresults_fixed import load_trialvault_results_to_database

        load_trialvault_results_to_database(results, results_url, debug_html_path)
    except Exception as e:
...
def parse_trialvault_event_scores_links(
    soup: BeautifulSoup,
    base_url: str = TRIALVAULT_BASE_URL,
) -> List[Tuple[str, str]]:
    """Extract (event_name, scores_url) pairs from a Trial Vault events page."""
    ...
    for row in soup.find_all("tr"):
        ...
        for link in links:
            ...
            if not href:
                continue
            if text == "results" or href.rstrip("/").endswith("/scores"):
                results_link = normalize_trialvault_results_url(urljoin(base_url, href))
            elif not event_name and text not in {"info", "results"}:
                event_name = link.get_text(strip=True)
        if not results_link or results_link in seen_urls:
            continue
        if not event_name:
            cells = [cell.get_text(strip=True) for cell in row.find_all(["td", "th"])]
            event_name = cells[0] if cells else results_link
        seen_urls.add(results_link)
        events.append((event_name, results_link))
    ...
    for link in soup.find_all("a", href=True):
        ...
        if "/events/" not in href:
            continue
        if not href.rstrip("/").endswith("/scores"):
            if re.search(r"/events/\d+/[^/]+/?$", href):
                href = href.rstrip("/") + "/scores"
            else:
                continue
        results_link = normalize_trialvault_results_url(urljoin(base_url, href))
        if results_link in seen_urls:
            continue
        event_name = link.get_text(strip=True) or results_link
        seen_urls.add(results_link)
        events.append((event_name, results_link))
...
def fetch_trialvault_event_catalog(session: requests.Session) -> List[Tuple[str, str]]:
    """Collect Trial Vault scores URLs from the archive and event catalog pages."""
    catalog_urls = [
        urljoin(TRIALVAULT_BASE_URL, "/events/archive"),
        urljoin(TRIALVAULT_BASE_URL, "/events/"),
    ]
    events: List[Tuple[str, str]] = []
    seen: set[str] = set()

    for page_url in catalog_urls:
        print(f"Fetching event catalog: {page_url}")
        html_content = fetch_trialvault_page(session, page_url)
        if not html_content:
            print(f"  WARNING: Could not load {page_url}")
            continue
        soup = BeautifulSoup(html_content, "html.parser")
        page_events = parse_trialvault_event_scores_links(soup)
        added = 0
        for event_name, results_url in page_events:
            if results_url in seen:
                continue
            seen.add(results_url)
            events.append((event_name, results_url))
            added += 1
        print(f"  Found {len(page_events)} links, {added} new")

    print(f"Total unique Trial Vault events: {len(events)}")
    return events


def scan_trialvault_debug_files(base_dir: str = ".") -> List[Tuple[str, str]]:
    """Find saved Trial Vault HTML files in year subfolders."""
    ...
    for item in sorted(os.listdir(base_dir)):
        ...
        if not os.path.isdir(item_path) or not year_folder_pattern.match(item):
            continue
        for filename in sorted(os.listdir(item_path)):
            if not filename.startswith("debug_trialvault_") or not filename.endswith(".html"):
                continue
            files.append((os.path.join(item_path, filename), item))
    return files


def apply_local_year_hint(results: List[TrialResult], year_hint: str) -> None:
    """Force parsed Trial Vault results to use the year folder they were loaded from."""
    if not year_hint or not re.fullmatch(r"\d{4}", str(year_hint).strip()):
        return
    folder_year = str(year_hint).strip()
    for result in results:
        if result.year != folder_year:
            result.year = folder_year


def filter_trialvault_local_files(
    local_files: List[Tuple[str, str]],
    file_filter: str = "",
    year_filter: str = "",
) -> List[Tuple[str, str]]:
    """Filter saved Trial Vault HTML files by optional filename substring and year folder."""
    filtered = local_files
    if year_filter:
        year = str(year_filter).strip()
        filtered = [(path, year_hint) for path, year_hint in filtered if year_hint == year]
    if file_filter:
        needle = file_filter.lower()
        filtered = [(path, year_hint) for path, year_hint in filtered if needle in path.lower()]
    return filtered


def parse_trialvault_html_file(
    file_path: str,
    year_hint: str = "",
    force_folder_year: bool = False,
) -> List[TrialResult]:
    """Parse a saved Trial Vault HTML file into trial results."""
    ...
    trial_name = "Unknown Trial"
    date_str = ""
    year = year_hint or str(time.localtime().tm_year)

    title_el = soup.find("h1", class_=re.compile("event-title")) or soup.find("h1")
    if title_el:
        trial_name = title_el.get_text(strip=True)
    canonical = soup.find("link", rel="canonical")
    results_url = canonical.get("href", "") if canonical else file_path
    if results_url:
        year = normalize_trial_year(year, trial_name=trial_name, results_url=results_url)
    else:
        year_match = re.search(r"(20\d{2})", trial_name)
        if year_match:
            year = year_match.group(1)
    ...
    if force_folder_year and year_hint:
        apply_local_year_hint(results, year_hint)
    return results


def scrape_trialvault_times_all(
    include_local_files: bool = True,
    local_only: bool = False,
) -> None:
    """Pull times from all Trial Vault events and append to existing placements."""
    print("=" * 80)
    print("TRIAL VAULT - TIMES-ONLY APPEND")
    print("=" * 80)
    print()

    from populate_trialresults_fixed import append_trialvault_times_to_database
    ...
    if not local_only:
        ...
        for idx, (event_name, results_url) in enumerate(events, start=1):
            print()
            print(f"[{idx}/{len(events)}] {event_name}")
            print(f"  URL: {results_url}")
            html_content = fetch_trialvault_page(session, results_url)
            if not html_content:
                print("  WARNING: Could not fetch scores page")
                continue

            trial_name, date_str, year = fetch_trialvault_event_metadata(session, results_url)
            if not trial_name or trial_name == "Unknown Trial":
                trial_name = event_name

            save_trialvault_debug_files(html_content, trial_name, year, results_url)
            soup = BeautifulSoup(html_content, "html.parser")
            results, _judges = parse_trialvault_scores_html(soup, trial_name, date_str, year)
            timed_results = [r for r in results if r.time]
            print(f"  Parsed {len(results)} placements, {len(timed_results)} with times")

            stats = append_trialvault_times_to_database(results, source_label=event_name)
            totals["events"] += 1
            for key in ("inserted", "matched", "skipped_has_time", "unmatched", "skipped_no_time"):
                totals[key] += stats.get(key, 0)

            time.sleep(0.5)

    if include_local_files or local_only:
        local_files = scan_trialvault_debug_files(".")
        if local_files:
            print()
            print(f"Processing {len(local_files)} local debug_trialvault HTML files...")
        for file_path, year_hint in local_files:
            print()
            print(f"Local file: {file_path}")
            results = parse_trialvault_html_file(file_path, year_hint, force_folder_year=True)
            timed_results = [r for r in results if r.time]
            print(f"  Parsed {len(results)} placements, {len(timed_results)} with times")
            stats = append_trialvault_times_to_database(
                results, source_label=os.path.basename(file_path)
            )
            totals["events"] += 1
            for key in ("inserted", "matched", "skipped_has_time", "unmatched", "skipped_no_time"):
                totals[key] += stats.get(key, 0)
    ...
    print()
    print("=" * 80)
    print("TRIAL VAULT TIMES APPEND COMPLETE")
    print(
        f"Processed {totals['events']} events/files | "
        f"inserted {totals['inserted']:,} | matched {totals['matched']:,} | "
        f"already had time {totals['skipped_has_time']:,} | "
        f"unmatched {totals['unmatched']:,}"
    )
    print("=" * 80)


def scrape_trialvault_nonsanctioned_local(
    file_filter: str = "",
    year_filter: str = "",
    purge_first: bool = False,
) -> None:
    """Append non-sanctioned Trial Vault placements from saved local HTML files."""
    from populate_trialresults_fixed import (
        append_trialvault_nonsanctioned_to_database,
        build_trial_list_lookup,
        get_connection,
        purge_trialvault_nonsanctioned_data,
        resolve_trial_list_id,
    )

    print("=" * 80)
    print("TRIAL VAULT - NON-SANCTIONED LOCAL APPEND")
    print("=" * 80)
    print()
    ...
    local_files = filter_trialvault_local_files(
        scan_trialvault_debug_files("."),
        file_filter=file_filter,
        year_filter=year_filter,
    )

    if not local_files:
        print("No matching local debug_trialvault HTML files found.")
        return

    if purge_first:
        print("Collecting trials to purge from local Trial Vault files...")
        conn = get_connection()
        cursor = conn.cursor()
        try:
        ...
        for file_path, year_hint in local_files:
            results = parse_trialvault_html_file(file_path, year_hint, force_folder_year=True)
            for result in results:
                if not is_nonsanctioned_trialvault_result(result):
                    continue
        ...
        print(f"Purging prior non-sanctioned TV data for {len(trial_ids)} trial(s)...")
        purge_trialvault_nonsanctioned_data(sorted(trial_ids))
        print()
    ...
    for file_path, year_hint in local_files:
        ...
        print(
            f"  Parsed {len(results)} placements, "
            f"{len(nonsanctioned)} non-sanctioned",
        )
        stats = append_trialvault_nonsanctioned_to_database(
            results, source_label=os.path.basename(file_path)
        )
        totals["files"] += 1
        for key in totals:
    ...
    print()
    print("=" * 80)
    print("TRIAL VAULT NON-SANCTIONED APPEND COMPLETE")
    print(
        f"Processed {totals['files']} files | "
        f"inserted {totals['inserted']:,} placements | "
        f"new classes {totals['classes']:,} | "
        f"times {totals['times_inserted']:,} | "
        f"duplicates {totals['skipped_duplicate']:,} | "
        f"no trial match {totals['skipped_no_trial']:,}"
    )
    print("=" * 80)


def is_nonsanctioned_trialvault_result(result: TrialResult) -> bool:
...
def scrape_trialvault_local_combined(
    file_filter: str = "",
    year_filter: str = "",
) -> None:
    """Append times and non-sanctioned placements from saved local Trial Vault HTML."""
    from populate_trialresults_fixed import (
        append_trialvault_nonsanctioned_to_database,
        append_trialvault_times_to_database,
    )

    print("=" * 80)
    print("TRIAL VAULT - LOCAL TIMES + NON-SANCTIONED APPEND")
    print("=" * 80)
    print()
    ...
    print(f"Processing {len(local_files)} local debug_trialvault HTML files...")
    time_totals = {"inserted": 0, "matched": 0, "unmatched": 0, "skipped_has_time": 0}
    ns_totals = {"inserted": 0, "classes": 0, "times_inserted": 0, "skipped_no_trial": 0}
    ...
    for file_path, year_hint in local_files:
        ...
        print(
            f"  Parsed {len(results)} placements, "
            f"{len(timed_results)} with times, "
            f"{len(nonsanctioned)} non-sanctioned",
        )

        time_stats = append_trialvault_times_to_database(
            results, source_label=os.path.basename(file_path)
        )
        for key in time_totals:
            time_totals[key] += time_stats.get(key, 0)

        ns_stats = append_trialvault_nonsanctioned_to_database(
            results, source_label=os.path.basename(file_path)
        )
        for key in ns_totals:
            ns_totals[key] += ns_stats.get(key, 0)

    print()
    print("=" * 80)
    print("TRIAL VAULT LOCAL COMBINED APPEND COMPLETE")
    print(
        f"Times: inserted {time_totals['inserted']:,}, matched {time_totals['matched']:,}, "
        f"unmatched {time_totals['unmatched']:,}, already had time {time_totals['skipped_has_time']:,}"
    )
    print(
        f"Non-sanctioned: inserted {ns_totals['inserted']:,}, new classes {ns_totals['classes']:,}, "
        f"times {ns_totals['times_inserted']:,}, no trial match {ns_totals['skipped_no_trial']:,}"
    )
    print("=" * 80)
...
def parse_local_trial_file(file_path: str, year: str = None, source_folder: str = None) -> Tuple[List[TrialResult], Dict[str, str]]:
    ...
    if not trial_name:
        ...
        trial_name = re.sub(r'^(debug_trialvault_|debug_trial_|debug_)', '', trial_name, flags=re.IGNORECASE)
...
def parse_individual_trial_page(url: str, trial_name: str, date_str: str, year: str) -> Tuple[List[TrialResult], Dict[str, str]]:
    ...
    if "archive" not in url.lower():
        year_dir, debug_file, raw_text_file = get_trial_debug_file_paths(trial_name, year)
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Scrape JRTCA and Trial Vault trial results")
    parser.add_argument(
        "--trialvault",
        action="store_true",
        help="Pull results from a single Trial Vault event (prompts for credentials and URL)",
    )
    parser.add_argument(
        "--trialvault-times",
        action="store_true",
        help="Pull times from all Trial Vault events and append to existing trial placements",
    )
    parser.add_argument(
        "--trialvault-times-local-only",
        action="store_true",
        help="Append times from saved debug_trialvault_*.html files only (no login)",
    )
    parser.add_argument(
        "--trialvault-nonsanctioned-local-only",
        action="store_true",
        help="Append non-sanctioned classes from saved debug_trialvault_*.html files only",
    )
    parser.add_argument(
        "--trialvault-nonsanctioned-fix-divisions",
        action="store_true",
        help="Purge mis-assigned non-sanctioned TV data, then reload from local HTML files",
    )
    parser.add_argument(
        "--trialvault-local-combined",
        action="store_true",
        help="Append times and non-sanctioned classes from saved debug_trialvault_*.html files",
    )
    parser.add_argument(
        "--trialvault-file",
        default="",
        help="Optional filename substring filter for local Trial Vault loads",
    )
    parser.add_argument(
        "--trialvault-year",
        default="",
    ...
    if args.trialvault_nonsanctioned_fix_divisions:
        scrape_trialvault_nonsanctioned_local(
            file_filter=args.trialvault_file,
            year_filter=args.trialvault_year,
            purge_first=True,
        )
        return

    if args.trialvault_nonsanctioned_local_only:
        scrape_trialvault_nonsanctioned_local(
            file_filter=args.trialvault_file,
            year_filter=args.trialvault_year,
        )
    ...
    if args.trialvault_local_combined:
        scrape_trialvault_local_combined(
            file_filter=args.trialvault_file,
            year_filter=args.trialvault_year,
        )
        return

    if args.trialvault_times or args.trialvault_times_local_only:
